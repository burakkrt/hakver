...
**Symmetric block 404 rule (authoritative):** Before any response shape is assembled, the service calls `BlockService.isBlocked(viewerId, topic.authorId)`. When `true` (and the topic is not anonymous), the endpoint responds with `404 Not Found` using the standard `{ code: "TOPIC_NOT_FOUND", message: "Konu bulunamadı" }` envelope. Returning 404 instead of 403 keeps the blocker/blocked relationship opaque to both parties — a blocked user cannot infer they were blocked from the status code. This rule applies to every interaction that resolves a topic by id or slug, so voting, commenting, bookmarking, and muting all surface the same "topic does not exist" response when the viewer is under a symmetric block.

**Authority-role bypass (authoritative):** Users holding an authority role (MODERATOR, ADMIN, or any future authority role) are **fully exempt** from the symmetric block check across every touch point — topic listing, topic detail, comment listing, moderation dashboards, admin user detail, and notification fan-out. A regular user may still block an authority account (the authority user's personal content simply stops appearing in the blocker's feed), but the authority user retains complete visibility into the blocker's content for moderation and KVKK oversight. Without this exemption a regular user could evade moderation by blocking every moderator; the earlier rule that only bypassed the check for the `user:view-anonymous` permission is superseded by this full role-based exemption. `BlockService.isBlocked(viewerId, targetId)` short-circuits to `false` whenever the viewer carries an authority role, so every downstream caller inherits the bypass without needing its own role check.
